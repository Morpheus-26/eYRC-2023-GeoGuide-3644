...
from direction_algo import dijkstra_shortest_path,turning_str,find_nearest_index
from Detection import detect
import socket
import sys	
import cv2
from cv2 import aruco
from excel import read_csv,write_csv,tracker
import time
import numpy as np
import math
...
key_id_mapping={"A":"5","B":"12","C":"13","D":"6","E":"7","0":"0"}
min_max_mapping={"A":(168,212),"B":(610,670),"C":(630,670),"D":(151,218),"E":(172,236),"0":(10,57)}

graph = {
    '0': [('1', 98.07)],
    '1': [('0', 98.07), ('2', 179.17), ('5', 110)],
    '2': [('1', 179.17), ('9', 364.35), ('3', 209)],
    '3': [('2', 209), ('6', 113), ('4', 143)],
    '4': [('3', 143), ('17', 141.43), ('11', 365.3)],
    '5': [('1', 1), ('8',247)],
    '6': [('3', 113), ('10', 258.23)],
    '7': [('17', 101), ('18', 450)],
    '8': [('5', 247), ('9', 189)],
    '9': [('2', 364.35), ('12', 225), ('10', 183), ('8', 189)],
    '10': [('6', 258.23), ('13', 257.15), ('9', 183), ('11', 135)],
    '11': [('4', 365.3), ('16', 365.26), ('10', 135)],
    '12': [('9', 225), ('14', 148)],
    '13': [('10', 257.15), ('15', 113)],
    '14': [('12', 148), ('15', 178)],
    '15': [('13', 113), ('14', 178), ('16', 134)],
    '16': [('11', 365.26), ('15', 134), ('18', 158.23)],
    '17': [('7', 101), ('4', 141.23)],
    '18': [('7', 549.76), ('16', 158.23)],
}
...
def cropping_v3(frame):
    ...
    x1, y1 = 135,38  # top-left
    x2, y2 = 531,35  # top-right
    x3, y3 = 531,432  # bottom-right
    x4, y4 = 134,430  # bottom-left
...
def cropping_v5(frame):
    ...
    x1, y1 = 96,39  # top-left
    x2, y2 = 531,35  # top-right
    x3, y3 = 531,432  # bottom-right
    x4, y4 = 96,430  # bottom-left
...
def cropping_v1(frame):
    ...
    x1, y1 = 551,111  # top-left
    x2, y2 = 1436,106  # top-right
    x3, y3 = 1442,998  # bottom-right
    x4, y4 = 543,993  # bottom-left
...
if __name__ == "__main__":
    ...
    ip = "192.168.47.126"     #Enter IP address of laptop after connecting it to WIFI hotspot
    nodearrived= False
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        with conn:
            ...
            sorted_keys = sorted(identified_labels.keys(), key=lambda x: priority_order.index(identified_labels[x]))      # Sort the keys of the identified_labels dictionary based on a custom priority order.
            ...
            while True:
                _,_img=cap.read()  
                img=cropping_v3(_img)                        # Crop the image using the cropping_v3 function.
                img=cv2.rotate(img,cv2.ROTATE_90_CLOCKWISE)  # Crop the image using the cropping_v3 function.
                img=cv2.resize(img, (893, 900))              # Resize the rotated image to a specified width and height
                show_img=cropping_v5(_img)
                show_img=cv2.rotate(show_img,cv2.ROTATE_90_CLOCKWISE)
                ...
                if(time.time()-curr_time>8):                 # Check if 8 seconds have passed since the last iteration
                    ...
                    ArUco_details_dict, ArUco_corners = detect_ArUco_details(img)               # Detect ArUco marker details and corners in the image.
                    ...
                    try:
                        ...
                        pass    
                    start_node=str(find_nearest_index({'x': ArUco_details_dict[100][0][0], 'y': ArUco_details_dict[100][0][1]}))  # Determine the start node based on the position of ArUco marker with ID 100.
                    ...
                    if(nodearrived=="ready"):                    # Check if the nodearrived signal indicates readiness for the next node.
                        ...
                        shortest_path = dijkstra_shortest_path(graph, start_node, end_node)  # Find the shortest path using Dijkstra's algorithm.
